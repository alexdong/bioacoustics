...
def load_and_compute_distance_matrix(
    ...
    assert all(
        col in taxonomy_df.columns for col in required_cols
...
def _build_lineage_map(
    ...
    df_filtered = df[df["primary_label"].isin(species_set)].drop_duplicates(
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"[DEMO] Dist(sp_a1, sp_b1) = {dist_matrix_demo[idx_a1, idx_b1]} (Expected: 4)",
        )
        print(
            f"[DEMO] Dist(sp_a1, sp_c1) = {dist_matrix_demo[idx_a1, idx_c1]} (Expected: 6)",
        )  # Different order
