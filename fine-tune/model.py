...
def build_model(
    ...
    whisper_config = WhisperConfig.from_pretrained(
        f"openai/whisper-{config.WHISPER_MODEL_SIZE}",
    )
    ...
    print(
        f"[MODEL] Loading base Whisper encoder: openai/whisper-{config.WHISPER_MODEL_SIZE}",
    )
    ...
    if encoder_checkpoint_path:
        print(
            f"[MODEL] Loading SSL fine-tuned encoder weights from: {encoder_checkpoint_path}",
        )
        assert (
            encoder_checkpoint_path.is_file()
        ...
        try:
            ...
            if missing_keys:
                print(
                    f"[WARNING] Missing keys when loading encoder state_dict: {missing_keys}",
                )
            if unexpected_keys:
                print(
                    f"[WARNING] Unexpected keys when loading encoder state_dict: {unexpected_keys}",
                )
            assert (
                not unexpected_keys
            ), "Refused to load state dict with unexpected keys."
            print("[MODEL] Encoder weights loaded successfully.")
        except Exception as e:
            print(
                f"[ERROR] Failed to load encoder checkpoint: {encoder_checkpoint_path}. Error: {e}",
            )
            raise RuntimeError("Failed loading encoder checkpoint") from e
    else:
        print(
            "[MODEL] Using standard Whisper pre-trained encoder weights (no SSL checkpoint).",
        )
...
class BirdClefClassifier(nn.Module):
    ...
    def __init__(
        ...
        print(
            f"[MODEL] Classifier head initialized: Linear({self.feature_size}, {self.num_classes})",
        )
