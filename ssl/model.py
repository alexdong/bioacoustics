...
def build_ssl_model() -> nn.Module:
    ...
    print(
        f"[SSL MODEL] Loading base Whisper encoder: openai/whisper-{ssl_config.WHISPER_MODEL_SIZE}",
    )
    whisper_config = WhisperConfig.from_pretrained(
        f"openai/whisper-{ssl_config.WHISPER_MODEL_SIZE}",
    )
    ...
    if ssl_config.BASE_ENCODER_CHECKPOINT_PATH:
        print(
            f"[SSL MODEL] WARNING: Loading BASE encoder weights from {ssl_config.BASE_ENCODER_CHECKPOINT_PATH} - implement loading logic if needed.",
        )
        # Implement loading logic similar to fine_tune/model.py if required

    # 2. Define Decoder
    print(
        f"[SSL MODEL] Defining Simple MAE Decoder (Depth: {ssl_config.DECODER_DEPTH}, Dim: {ssl_config.DECODER_EMBED_DIM})",
    )
...
class SimpleMAEDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, masked_indices: Tensor) -> Tensor:
        ...
        mask_expanded = masked_indices.unsqueeze(-1).to(
            x.dtype,
        )  # (batch, total_frames, 1)
...
class MaskedAutoencoderModel(nn.Module):
    ...
    def forward(self, masked_features: Tensor, mask: Tensor) -> Tensor:
        ...
        if encoder_frame_count != input_frame_count:
            print(
                f"[WARNING] Frame count mismatch: encoder={encoder_frame_count}, input={input_frame_count}",
            )
            # Two options:
            ...
            resized_mask = torch.nn.functional.interpolate(
                ...
                1,
            )  # Remove channel dim: (B, N_FRAMES_ENC)
        ...
        assert (
            predictions.shape[1] == encoder_frame_count
        ), "Frame count mismatch in output"
...
if __name__ == "__main__":
    ...
    n_frames_demo = (
        int(
            ssl_config.CHUNK_DURATION_SEC
            * ssl_config.TARGET_SAMPLE_RATE
            / ssl_config.HOP_LENGTH,
        )
        + 1
    )
    ...
    try:
        ...
        dummy_mask = torch.rand(batch_size_demo, n_frames_demo) > (
            1.0 - ssl_config.MASKING_RATIO
        )  # Boolean mask
        ...
        print(
            "[DEMO] NOTE: Output shape assertion commented out due to potential frame mismatch from Whisper encoder conv layers. Verify manually.",
        )
