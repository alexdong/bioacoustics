...
def load_inference_model(
    ...
    if model_format == "pytorch":
        ...
        assert (
            quantized_path is not None
        ), "Path to quantized model must be provided for 'int8' format."
        assert (
            quantized_path.is_file()
        ), f"Quantized model file not found: {quantized_path}"
        model = _load_quantized_model(quantized_path, num_classes, device)
    elif model_format == "torchscript":
        assert (
            torchscript_path is not None
        ), "Path to TorchScript model must be provided for 'torchscript' format."
        assert (
            torchscript_path.is_file()
        ), f"TorchScript model file not found: {torchscript_path}"
...
def _load_pytorch_model(
    ...
    try:
        ...
        if missing_keys or unexpected_keys:
            print(
                f"[WARNING] State dict mismatch! Missing: {missing_keys}, Unexpected: {unexpected_keys}",
            )
...
def _load_quantized_model(
    ...
    print(
        f"[MODEL LOAD - INT8] WARNING: Assuming dynamic quantization applied on the fly. Provided path '{quantized_model_path}' might not be directly loaded if it's not an FP32 checkpoint.",
    )
...
def _load_torchscript_model(
    ...
    assert (
        torchscript_model_path.is_file()
    ), f"TorchScript file not found: {torchscript_model_path}"
        ...
        if dummy_fp32_path:
            try:
                ...
                try:
                    ...
                    ts_model = load_inference_model(
                        "torchscript",
                        dummy_fp32_path,
                        dummy_device,
                        torchscript_path=dummy_ts_path,
                    )
