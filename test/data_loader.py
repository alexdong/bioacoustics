    ...
    if metadata_path:
        ...
        for _, row in test_df.iterrows():
            ...
            try:
                ...
                print(
                    f"[ERROR] Could not parse row_id: {row_id}. Error: {e}. Skipping.",
                )
        ...
        print(
            f"[WARNING] No test metadata provided. Globbing {test_audio_dir} and creating 5s chunks.",
        )
        ...
        for audio_path in all_test_files:
            try:
                ...
                for i in range(num_chunks):
                    ...
                    row_id = (
                        f"{audio_path.stem}_{int(end_time_sec)}"  # Construct row_id
                    )
                    test_metadata.append(
                        {
                            "row_id": row_id,
...
class TestDataset(Dataset[Tuple[AudioTensor, RowID]]):
    ...
    def __init__(
        ...
        metadata: List[
            Dict[str, Any]
        ],  # List of {'row_id', 'filename', 'start_sec', 'end_sec'}
        audio_dir: Path,
    ) -> None:
        super().__init__()
        ...
        print(
            f"[TEST DATASET] Initialized Test Dataset with {len(self.metadata)} chunks.",
        )
    ...
    def __getitem__(self, index: int) -> Tuple[AudioTensor, RowID]:
        ...
        row[
            "end_sec"
        ]  # This might slightly differ from start_sec + 5 if duration wasn't multiple of 5
        ...
        try:
            ...
            print(
                f"[WARNING] Failed loading segment for {row_id} from {audio_path}. Error: {e}. Returning silence.",
            )
        ...
        # Ensure exact length
        assert (
            waveform.shape[0] == self.chunk_samples
        ), f"Waveform length incorrect for {row_id}"
...
if __name__ == "__main__":
    ...
    dummy_metadata_data = {
        "row_id": [
            f"{dummy_audio_path.stem}_5",
            f"{dummy_audio_path.stem}_10",
            f"{dummy_audio_path.stem}_15",
        ],
    ...
    try:
        ...
        print(
            f"[DEMO]   Batch Mels shape: {batch_mels_meta.shape}",
