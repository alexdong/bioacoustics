...
if not json_files:
    print(
        f"Error: Python glob could not find any files matching pattern '{json_pattern}'",
    )
    exit()
...
try:
    print(
        f"Creating table '{table_name}' schema based on first file: {os.path.basename(json_files[0])}",
    )
    # Use read_json_auto on a single file list parameter
    con.execute(
        f"CREATE OR REPLACE TABLE {table_name} AS SELECT * FROM read_json_auto(?) LIMIT 0",
        [[json_files[0]]],
    )
...
print(
    f"Starting data insertion in {num_batches} batches of up to {batch_size} files each...",
)
...
for i in range(0, total_files, batch_size):
    ...
    print(
        f"Processing Batch {current_batch_num}/{num_batches} ({len(batch_files)} files starting with {os.path.basename(batch_files[0])}...)",
        end="",
        flush=True,
    )
