...
if not INPUT_FOLDER.is_dir():
    print(
        f"Error: Input folder '{INPUT_FOLDER}' not found or is not a directory.",
        file=sys.stderr,
    )
...
try:
    ...
    with open(OUTPUT_CSV_FILE, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for json_file_path in tqdm(
            ...
            try:
                ...
                print(
                    f"\nWarning: Could not read file {json_file_path.name}: {e}",
                    file=sys.stderr,
                )
                error_files_count += 1
                ...
                print(
                    f"\nWarning: Unexpected error processing file {json_file_path.name}: {e}",
                    file=sys.stderr,
                )
                error_files_count += 1
    ...
    print(
        f"\nError: Could not open or write to output file {OUTPUT_CSV_FILE}: {e}",
        file=sys.stderr,
    )
...
print(
    f"Records matching filter ('{FILTER_KEY}' = '{FILTER_VALUE}') and extracted: {extracted_records_count}",
)
print(
    f"Files skipped due to errors (invalid JSON, read errors, etc.): {error_files_count}",
)
