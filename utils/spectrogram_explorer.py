...
class SpectrogramExplorer(QMainWindow):
    ...
    @Slot()
    def on_frame_length_changed(self) -> None:
    ...
    @Slot()
    def on_hop_length_changed(self) -> None:
    ...
    def load_audio_file(self) -> None:
        ...
        if file_path:
            ...
            print(
                f"Loaded: {os.path.basename(file_path)} | SR: {self.sr} Hz | Duration: {len(self.y)/self.sr:.2f}s",
            )
    ...
    def compute_spectrogram(self, frame_length: int, hop_length: int, scale: str, n_mels: int) -> Optional[np.ndarray]:
        if self.y is not None:
            if scale == "mel":
                ...
                S = np.abs(
                    librosa.stft(self.y, n_fft=frame_length, hop_length=hop_length),
                )
    ...
    def update_spectrogram(self) -> None:
        if self.y is None:
            return
