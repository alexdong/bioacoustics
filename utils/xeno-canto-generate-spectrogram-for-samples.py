...
def run_cli_command(command_list, command_desc="Command", timeout=300):
    ...
    try:
        process = subprocess.run(command_list, check=True, capture_output=True, text=True, timeout=timeout)
        return True, ""
        ...
        stderr_preview = (e.stderr or "No stderr captured")[:1000] + ('...' if e.stderr and len(e.stderr) > 1000 else '')
        error_msg = f"{command_desc} failed (Code {e.returncode})"
        # print(error_msg)
        ...
        error_msg = f"Unexpected error running {command_desc}: {e}"
        # print(error_msg)
        # print(f"  Command: {' '.join(command_list)}")
        return False, f"Unexpected Error: {e}"


def generate_spectrogram(audio_path, output_png_path, sample_rate, n_fft, hop_length, n_mels, fmin, fmax, power):
    ...
    try:
        ...
        amplitude_to_db_transform = torchaudio.transforms.AmplitudeToDB(stype='power', top_db=80)
        ...
        img = ax.imshow(plot_spec, aspect='auto', origin='lower', cmap='magma')
        plt.axis('off')
        ...
        return False, "Audio file not found"
         ...
         err_str = str(e).lower()
         if "requires output shape of" in err_str or "invalid audio data" in err_str or "input tensor must have" in err_str or "frame_length must be" in err_str:
              return False, f"Torchaudio Runtime Error (Likely too short/corrupt)"
         else:
...
def download_s3_file(recording_id, json_dir, s3_bucket_prefix, mp3_dir, skip_existing=True):
    ...
    if skip_existing and os.path.exists(local_mp3_path) and os.path.getsize(local_mp3_path) > 0:
        status["success"] = True
        status["skipped"] = True
    ...
    try:
        ...
        download_command = ['aws', 's3', 'cp', '--only-show-errors', s3_path, local_mp3_path]
        ...
        if success:
            ...
            if os.path.exists(local_mp3_path) and os.path.getsize(local_mp3_path) > 0:
                 ...
                 status["reason"] = "Download command succeeded but file is missing/empty"
                 if os.path.exists(local_mp3_path): # Clean up zero-byte file
                      try: os.remove(local_mp3_path)
                      except OSError: pass
        else:
...
def process_audio_files(rec_id, species_name, mp3_dir, output_dir, temp_dir):
    ...
    status = {"id": rec_id, "species": species_name, "success": False, "reason": "",
              "segments_created": 0, "specs_created": 0, "spec_errors": 0}
    ...
    convert_command = [
        'ffmpeg', '-i', mp3_path, '-y', '-hide_banner', '-loglevel', 'error',
        '-ac', '1', '-ar', str(TARGET_SAMPLE_RATE),
        '-c:a', 'libvorbis', '-q:a', '4', temp_ogg_path
    ]
    ...
    segment_command = [
        'ffmpeg', '-i', temp_ogg_path, '-y', '-hide_banner', '-loglevel', 'error',
        '-f', 'segment', '-segment_time', str(SEGMENT_DURATION),
        '-c', 'copy', '-reset_timestamps', '1', segment_pattern
    ]
    ...
    for segment_file in segments_found:
        ...
        spec_success, spec_reason = generate_spectrogram(
            segment_path, spec_path, TARGET_SAMPLE_RATE,
            N_FFT, HOP_LENGTH, N_MELS, FMIN, FMAX, POWER
        )
        if spec_success:
    ...
    if status["segments_created"] == 0 and status["success"]: # Check again after finding segments
         status["success"] = False
...
# --- Main Execution ---
def main():
    parser = argparse.ArgumentParser(description="Download S3 files, convert, segment, generate spectrograms in parallel.")
    parser.add_argument("--input-list-file", default="datasets/xeno-canto-brazil-small.txt", help="Path to the text file containing 'species|recording_id1,id2,...'")
    parser.add_argument("--json-dir", default="./datasets/xeno-canto/", help="Directory containing the individual recording JSON metadata files.")
    parser.add_argument("--mp3-dir", default="./datasets/xeno-canto-brazil-small", help="Directory to save downloaded MP3 files.")
    parser.add_argument("--output-dir", default="./datasets/xeno-canto-brazil-small", help="Directory to save the final segmented OGG and spectrogram PNG files.")
    parser.add_argument("--max-workers", type=int, default=None, help="Max number of parallel processes (default: number of CPU cores).")
    parser.add_argument("--limit", type=int, default=None, help="Limit processing to the first N recordings (for testing).")
    parser.add_argument("--skip-existing-mp3", action='store_true', help="Skip downloading MP3s if they already exist.")
    parser.add_argument("--skip-download", action='store_true', help="Skip the S3 download phase entirely (assumes MP3s exist).")
    ...
    try:
        with open(args.input_list_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                species_name = species_part.strip().replace(' ', '_').replace('/', '_').replace('\\', '_')
                if ids_part:
                    ids_list = [rec_id.strip() for rec_id in ids_part.split(',') if rec_id.strip()]
    ...
    if not args.skip_download:
        ...
        for rec_id in sorted_ids:
            ...
            if rec_id in id_to_species_map:
                 download_tasks.append((rec_id, args.json_dir, S3_BUCKET_PREFIX, args.mp3_dir, args.skip_existing_mp3))
            else:
                 print(f"Warning: No species mapping for {rec_id}, cannot determine JSON path if needed. Skipping download.")
        ...
        with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
            futures = {executor.submit(download_s3_file, *task): task for task in download_tasks}
            for future in tqdm(as_completed(futures), total=len(download_tasks), desc="Downloading MP3s"):
                try:
                    ...
                    if not result["success"]:
                        download_errors += 1
                        if result["skipped"]:
                            download_skips += 1 # Count skips separately from actual errors
                            download_errors -=1 # Don't count skip as error
                        # else:
                        #     print(f"\nDownload Error (ID: {result['id']}): {result['reason']}")
                except Exception as e:
                    rec_id = futures[future][0]
                    download_results.append({"id": rec_id, "success": False, "reason": f"Task level error: {e}", "skipped": False})
                    download_errors += 1
                    # print(f"\nDownload Task Error (ID: {rec_id}): {e}")

        print("\n--- Download Summary ---")
        print(f"Tasks attempted: {len(download_tasks)}")
        successful_downloads = sum(1 for r in download_results if r["success"] and not r["skipped"])
        ...
        successfully_downloaded_or_skipped_ids = {r['id'] for r in download_results if r['success']}
        recordings_to_process = recordings_to_process.intersection(successfully_downloaded_or_skipped_ids)
        print(f"Proceeding to process {len(recordings_to_process)} recordings with available MP3s.")
        if not recordings_to_process:
             print("No recordings available for processing after download phase. Exiting.")
             sys.exit(0)
    else:
        print("\n--- Skipping S3 Download Phase ---")
        # Assume all requested IDs have MP3s available
        print(f"Proceeding to process {len(recordings_to_process)} recordings (assuming MP3s exist).")
    ...
    audio_tasks = []
    for rec_id in sorted(list(recordings_to_process)): # Process only those confirmed available
        species_name = id_to_species_map.get(rec_id)
        if not species_name:
             print(f"Warning: No species name found for processing recording ID {rec_id}. Skipping.")
             continue
        audio_tasks.append((rec_id, species_name, args.mp3_dir, args.output_dir, audio_temp_dir))

    audio_results = []
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {executor.submit(process_audio_files, *task): task for task in audio_tasks}
        for future in tqdm(as_completed(futures), total=len(audio_tasks), desc="Processing Audio"):
            try:
                result = future.result()
                ...
                audio_results.append({"id": rec_id, "species": "unknown", "success": False, "reason": f"Task level error: {e}",
                                "segments_created": 0, "specs_created": 0, "spec_errors": 0})
