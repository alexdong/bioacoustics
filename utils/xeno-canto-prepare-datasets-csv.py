...
def create_splits(input_dir, output_dir, test_size=0.1, val_size=0.1, random_state=42):
    """
    ...
    for filepath in input_path.rglob("*.png"):
        ...
        if parsed:
            ...
            if recordings_data[rec_id]["species"] is None:
                ...
                logging.warning(f"Recording ID {rec_id} found with multiple species: "
    ...
    logging.info(f"Found {len(all_files_data)} total segments from {len(recordings_data)} unique recordings.")
    ...
    recording_species = [recordings_data[rec_id]["species"] for rec_id in unique_recording_ids]
    ...
    try:
        ...
        logging.info(f"Split {len(test_ids)} recordings for test set.")
         ...
         logging.error(f"Could not perform initial train/test split, possibly due to small classes: {e}")
         # Handle case where stratification might fail with very small classes
         if 'n_splits=2' in str(e):
              logging.warning("Attempting split without stratification due to small classes...")
              ...
              train_val_species = [recordings_data[rec_id]["species"] for rec_id in train_val_ids]
              logging.info(f"Split {len(test_ids)} recordings for test set (no stratification).")
         else:
              raise e # Re-raise other errors


    # --- Split 2: Separate Train and Validation Sets ---
    if len(train_val_ids) > 1 and relative_val_size > 0: # Need at least 2 samples to split
        try:
            ...
            logging.info(f"Split {len(train_ids)} recordings for train, {len(val_ids)} for validation.")
        except ValueError as e:
            logging.error(f"Could not perform train/validation split, possibly due to small classes: {e}")
            if 'n_splits=2' in str(e):
                logging.warning("Attempting split without stratification due to small classes...")
                ...
                logging.info(f"Split {len(train_ids)} recordings for train, {len(val_ids)} for validation (no stratification).")
            else:
                # If split fails even without strat, assign all remaining to train
                logging.warning("Assigning all remaining train_val samples to train set.")
        ...
        logging.warning("Not enough data or val_size=0, assigning remaining to train set.")
        train_ids = train_val_ids
        val_ids = []
        logging.info(f"Split {len(train_ids)} recordings for train, {len(val_ids)} for validation.")
    else:
    ...
    def get_split_data(recording_ids):
        ...
        for rec_id in recording_ids:
            ...
            for fpath in recordings_data[rec_id]["files"]:
                split_files.append({"filepath": fpath, "species": species, "label": label, "recording_id": rec_id})
        return split_files
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Split bird song spectrogram dataset by recording ID.")
    parser.add_argument(
        "--input_dir",
        type=str,
