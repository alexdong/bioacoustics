                ...
                try:
                    ...
                    if species_name not in species_agg_data:
                        species_agg_data[species_name] = {
                            ...
                            'quality_seconds': collections.defaultdict(int) # Store seconds per quality
                        }

                    # Update aggregates for this species
                    species_agg_data[species_name]['total_duration_seconds'] += duration_seconds
                    if quality: # Only add to quality breakdown if quality exists
                        species_agg_data[species_name]['quality_seconds'][quality] += duration_seconds
...
def write_aggregated_csv(output_path, species_data, quality_codes):
    """
...
if __name__ == "__main__":
    ...
    if aggregated_data is not None and unique_qualities is not None:
        print(f"Aggregation complete. Found {len(aggregated_data)} unique species entries.")
        print(f"Unique quality codes found: {sorted(list(unique_qualities))}")

        # Write the aggregated data to the output CSV
        success = write_aggregated_csv(output_csv_file, aggregated_data, unique_qualities)
