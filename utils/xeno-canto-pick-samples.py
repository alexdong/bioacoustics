...
try:
    with open(SUMMARY_CSV_PATH, 'r', encoding='utf-8') as f_csv:
        ...
        for row in reader:
            ...
            if (usable_seconds >= MIN_USABLE_SECONDS and
                species_name.lower() != 'identity unknown' and
                species_name): # Ensure name is not empty
...
print(f"Found {len(eligible_species)} eligible species meeting the >= {MIN_USABLE_SECONDS}s threshold.")
...
if len(eligible_species) < TARGET_SPECIES_COUNT:
     print(f"WARNING: Fewer than {TARGET_SPECIES_COUNT} eligible species available ({len(eligible_species)}). Will select all available.")
...
print(f"  Tier Medium ({TIER_LOW_UPPER_BOUND}s - {TIER_MED_UPPER_BOUND}s): {len(tier_med_spp)} species")
print(f"  Tier Low ({MIN_USABLE_SECONDS}s - {TIER_LOW_UPPER_BOUND}s): {len(tier_low_spp)} species")
...
species_to_ids = {species: [] for species in selected_species_en} # Pre-populate dictionary
...
try:
    ...
    for entry in os.scandir(JSON_DIR_PATH):
        ...
        if files_scanned % 10000 == 0: # Print progress update
             ...
             print(f"  Scanned {files_scanned} files... ({rate:.0f} files/sec). Found {relevant_found} relevant. Errors: {errors_encountered}", end='\r')
...
with open(OUTPUT_FILE_PATH, 'w', encoding='utf-8') as f_out:
    for species_name in selected_species_en: # Iterate using the sorted list
        ids_list = species_to_ids.get(species_name, []) # Get list, default to empty if somehow missing
        ids_string = ','.join(sorted(ids_list)) # Join IDs into comma-separated string, sort for consistency
        f_out.write(f"{species_name}|{ids_string}\n")
        lines_written += 1
        if not ids_list:
             print(f"  WARNING: No Brazil recordings found in metadata for selected species: {species_name}")


print(f"\nProcess complete. Wrote {lines_written} species mappings to {OUTPUT_FILE_PATH}")
if lines_written != len(selected_species_en):
     print(f"WARNING: Mismatch in lines written ({lines_written}) vs species selected ({len(selected_species_en)}). Check warnings.")
